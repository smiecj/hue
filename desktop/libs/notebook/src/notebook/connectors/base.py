...
def get_api(request, snippet):
  ...
  if interface.startswith('hiveserver2') or interface == 'hms':
    ...
    elif interpreter['options'] and interpreter['options'].get('url', '').find('mysql') >= 0:
      from notebook.connectors.jdbc_mysql import JdbcApiMySQL
      return JdbcApiMySQL(request.user, interpreter=interpreter)
    else:
